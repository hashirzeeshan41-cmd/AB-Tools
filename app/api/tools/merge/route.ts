...
export async function POST(req: Request) {
  try {
    ...
    // create processing job (attach to first file for reference)
    const job = await prisma.processingJob.create({ data: { fileId: fileIds[0], tool: 'merge', status: 'pending' } })

    // enqueue job
    await queue.add('merge', { processingJobId: job.id, fileIds })
